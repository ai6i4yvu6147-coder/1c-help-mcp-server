...
| ~~`validate_code`~~ | **Убран** — эвристика давала массовые false positive (см. ниже) |
...
| `validate` | Конструктор: XML-структура метаданных + наличие обработчиков команд/событий (BSL-проверка по справке отключена) |
...
2. `set_dcs(project=..., query=..., fields=[...], standard_period=true, totals=[{"data_path":"Часы","expression":"Сумма(Часы)"}], layout={"group_by":[{"field":"Исполнитель"},{"field":"Задача"}], "selection":[...]})`
   ...
   - **Условное оформление** (`layout.conditional_appearance`) — цвет/шрифт/формат/текст поля по условию (самая частая фича реальных отчётов): `[{"fields":["СуммаОстаток"],"filter_items":[{"field":"СуммаОстаток","comparison":"Less","value":0,"value_type":"xs:decimal"}],"appearance":[{"parameter":"ЦветТекста","value":"#FF0000"}]}]`. Параметры/типы — `describe(unit='dcs', name='conditional_appearance')`; цвет — `"#RRGGBB"`/`"style:Имя"`, шрифт — dict `{ref,height,bold,...}`.
   - **Группы отборов** (И/ИЛИ/НЕ): в `filter_items` наравне с обычными отборами кладите `{"group_type":"OrGroup","items":[<обычные отборы или вложенные группы>],"presentation_ru":"...","generate_user_setting_id":true}` — одна пользовательская галка над ИЛИ-набором условий. Контракт — `describe(unit='dcs', name='filter_group')`.
...
### `validate_code` убран (эвристическая BSL-проверка отключена)

Прежний `validate_code` (и BSL-ветка конструкторского `validate`) лексически ловил каждый
вызов `Получатель.Метод(` и выдавал `unknown_object`, если получатель не найден в справке.
Но получатель почти всегда — локальная переменная / параметр / реквизит формы или объекта
(`ТЗ = Новый ТаблицаЗначений; ТЗ.Добавить()`, `Запрос.Выполнить().Выбрать()`, …), а трекинга
типов переменных не было — поэтому после загрузки справки проверка давала ~50 ложных
срабатываний на 1 реальную ошибку и как сигнал была бесполезна.

- MCP-инструмент `validate_code` **удалён** из поверхности.
- `HelpTools.validate_code` оставлен единой точкой-заглушкой (возвращает `[]`) под будущий
  полноценный линтер; конструкторский `validate` его вызывает, поэтому `bsl_errors` сейчас
  всегда пуст, но линтер подхватится автоматически, когда заглушку заменят.
- Что `validate(project=…)` проверяет сейчас: XML-структуру метаданных (библиотечный
  `validate()`) и наличие процедур-обработчиков команд/событий формы.
