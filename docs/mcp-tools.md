...
| `describe` | Reflection-справка конструктора: `unit` (`dcs`) + `name` (13 разделов: `dataset`/`dataset_link`/`field`/`role`/`calculated_field`/`total_field`/`parameter`/`output_parameter`/`filter`/`selection`/`order`/`layout`/`conditional_appearance`); без `name` — обзор. Поля/enum/пример для сборки payload. Не путать со справкой BSL |
