...
Human-tier — русский OK. В хаб-модели (canon 2.6.0) синхронизация протокола идёт через `GROUP-HUB.md`; общий канон группы Sub читает на Head в `docs/group/shared/` (`head.path`) — **локальной копии и snapshot-install нет**. Оператор **не копирует пакеты** между репозиториями. За оператором остаются вещи вне контекста агента: деплой portable MCP.
